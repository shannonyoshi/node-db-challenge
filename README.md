...
A relational database holds data that relates to each ther in tables. SQL (structured query language) allows us to get the information we want from the database using queries.

1. Why do tables need a `primary key`?

We need a primary key to uniquely identify each entry in the database
...
Have a third table which has foreign keys for the original 2 tables 
